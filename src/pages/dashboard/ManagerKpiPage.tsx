...
export function ManagerKpiPage() {
  ...
  const profileQuery = useQuery({
    queryKey: ["profile", "me", "manager-kpi"],
  ...
  const employeesQuery = useQuery({
    queryKey: ["manager-kpi-employees-select", departmentId],
  ...
  const kpiQuery = useQuery({
    queryKey: ["manager-kpi-by-employee", employeeIdApplied, page, size],
    ...
    <section className="mx-auto max-w-6xl space-y-5">
      ...
      <Card className="border-border/70 bg-card/95">
        ...
        <CardContent>
          ...
          {profileQuery.isLoading || employeesQuery.isLoading ? (
            <Message type="info">
      ...
      <Card className="border-border/70 bg-card/95">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor="manager-kpi-page-size">{tr("Page size", "Sahifa hajmi", "Р Р°Р·РјРµСЂ СЃС‚СЂР°РЅРёС†С‹")}</Label>
            <Select
              id="manager-kpi-page-size"
              value={String(size)}
              onChange={(event) => {
