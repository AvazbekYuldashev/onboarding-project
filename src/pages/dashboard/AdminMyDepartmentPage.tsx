...
export function AdminMyDepartmentPage() {
  const { language } = useI18n();
  const tr = (en: string, uz: string, ru: string) =>
    language === "UZ" ? uz : language === "RU" ? ru : en;

  const departmentQuery = useQuery({
    queryKey: ["department-core", "by-my", "admin"],
    ...
    <section className="mx-auto max-w-6xl space-y-5">
      <Card className="border-border/80 bg-card/95 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>{tr("Admin my department module.", "Admin mening bo'limim moduli.", "Модуль моего отдела администратора.")}</CardDescription>
        </CardHeader>
      </Card>
                  ...
                  <tr key={item.id} className="border-t border-border/70">
                    ...
                    <td className="px-3 py-2">
                      {chiefQuery.isLoading
                        ? tr("Loading...", "Yuklanmoqda...", "Загрузка...")
                        : chiefQuery.data
                          ? `${chiefQuery.data.name} ${chiefQuery.data.surname}`.trim() || chiefQuery.data.username || item.chiefId || "-"
                          : item.chiefId || "-"}
                    </td>
                    <td className="px-3 py-2">
