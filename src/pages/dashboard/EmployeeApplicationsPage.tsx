...
import {
  getEmployeeAcceptedApplications,
  getEmployeeStatusAcceptApplications,
  updateEmployeeApplicationStatus,
} from "@/api/applicationEmployeeApi";
...
const EMPLOYEE_ACTION_STATUSES: ApplicationStatus[] = ["IN_PROGRESS", "REVIEW", "DENIED"];
...
export function EmployeeApplicationsPage() {
  ...
  const [mode, setMode] = useState<"incoming" | "my">("incoming");
  const [selectedApplication, setSelectedApplication] = useState<ApplicationResponseDTO | null>(null);
  const [actionMessage, setActionMessage] = useState("");
  const [actionError, setActionError] = useState("");

  const applicationsQuery = useQuery({
    queryKey: ["employee-applications", mode, page, size],
    queryFn: () =>
      mode === "incoming"
        ? getEmployeeStatusAcceptApplications(page, size)
        : getEmployeeAcceptedApplications(page, size),
    staleTime: 30_000,
  });

  const updateStatusMutation = useMutation({
    mutationFn: updateEmployeeApplicationStatus,
    onSuccess: (message) => {
      setActionError("");
    ...
    onError: (error) => {
      setActionMessage("");
      setActionError(error instanceof Error ? error.message : tr("Failed to update status.", "Holatni yangilab bo'lmadi.", "Не удалось обновить статус."));
    },
  });

  const departmentDetailQuery = useQuery({
    queryKey: ["employee-application-department", selectedApplication?.departmentId],
  ...
  const buildingDetailQuery = useQuery({
    queryKey: ["employee-application-building", selectedApplication?.buildingId],
  ...
  const categoryDetailQuery = useQuery({
    queryKey: ["employee-application-category", selectedApplication?.categoryId],
  ...
  const offeringDetailQuery = useQuery({
    queryKey: ["employee-application-offering", selectedApplication?.offeringId],
  ...
  const senderDetailQuery = useQuery({
    queryKey: ["employee-application-sender", selectedApplication?.sendProfileId],
  ...
  const acceptorDetailQuery = useQuery({
    queryKey: ["employee-application-acceptor", selectedApplication?.acceptorProfileId],
    queryFn: () => getEmployeeById(selectedApplication?.acceptorProfileId ?? ""),
    enabled: Boolean(selectedApplication?.acceptorProfileId),
    retry: false,
  });

  const saveStatus = (application: ApplicationResponseDTO, next: ApplicationStatus) => {
    if (!EMPLOYEE_ACTION_STATUSES.includes(next)) return;
    updateStatusMutation.mutate({ id: application.id, status: next });
  };
    ...
    <section className="mx-auto max-w-6xl space-y-5">
      <Card className="border-border/80 bg-card/95 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>
            {tr("Employee applications module.", "Xodim arizalar moduli.", "Модуль заявок сотрудника.")}
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card className="border-border/70 bg-card/95">
        <CardHeader>
          ...
          <CardDescription>
            <span className="mr-2">
              {mode === "incoming"
                ? tr("Incoming to accept", "Qabul uchun kelganlar", "Входящие для принятия")
                : tr("Accepted by me", "Qabul qilganlarim", "Принятые мной")}
            </span>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Button type="button" variant={mode === "incoming" ? "default" : "outline"} onClick={() => { setMode("incoming"); setPage(1); }}>
              {tr("Incoming to accept", "Qabul uchun kelganlar", "Входящие для принятия")}
            </Button>
            <Button type="button" variant={mode === "my" ? "default" : "outline"} onClick={() => { setMode("my"); setPage(1); }}>
              {tr("Accepted by me", "Qabul qilganlarim", "Принятые мной")}
            </Button>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-muted/40 text-left">
                <tr>
                  ...
                  <th className="px-3 py-2">{tr("Details", "Batafsil", "Подробнее")}</th>
                </tr>
              </thead>
              <tbody>
                {applications.map((application) => (
                  <tr key={application.id} className="border-t border-border/70">
                    <td className="px-3 py-2">
                      ...
                      <div className="max-w-[18rem] truncate text-xs text-muted-foreground">{application.description || "-"}</div>
                    </td>
                    <td className="px-3 py-2">
                      <div className="space-y-2">
                        <Badge variant={getStatusVariant(application.status)}>{statusLabel(application.status, tr)}</Badge>
                        <Select
                          className="min-w-[155px]"
                          value={application.status ?? "SENT"}
                          disabled={updateStatusMutation.isPending}
                          onChange={(event) => saveStatus(application, event.target.value as ApplicationStatus)}
                        >
                          <option value={application.status ?? "SENT"}>{statusLabel(application.status, tr)}</option>
                          {EMPLOYEE_ACTION_STATUSES.filter((item) => item !== application.status).map((item) => (
                            <option key={item} value={item}>{statusLabel(item, tr)}</option>
                          ))}
                        </Select>
                      </div>
                    </td>
                    <td className="px-3 py-2">{application.offeringTitle || "-"}</td>
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor="employee-applications-page-size">{tr("Page size", "Sahifa hajmi", "Размер страницы")}</Label>
            <Select id="employee-applications-page-size" value={String(size)} onChange={(event) => { setSize(Number(event.target.value)); setPage(1); }}>
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Button type="button" variant="outline" disabled={page >= totalPages || applicationsQuery.isFetching} onClick={() => setPage((previous) => Math.min(previous + 1, totalPages))}>
                {tr("Next", "Keyingi", "Вперёд")}
              </Button>
            </div>
          </div>
        </CardContent>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-3xl border-border/80 bg-card shadow-lg">
            ...
            <CardContent className="grid gap-2 md:grid-cols-2">
              ...
              <DetailItem label={tr("Deadline", "Deadline", "Deadline")} value={selectedApplication.deadline !== undefined && selectedApplication.deadline !== null ? `${selectedApplication.deadline} soat` : "-"} />
              <div className="md:col-span-2">
                <DetailItem label={tr("Description", "Tavsif", "Описание")} value={selectedApplication.description || "-"} />
              </div>
              <div className="md:col-span-2">
                <DetailItem label={tr("Comments", "Izoh", "Комментарий")} value={selectedApplication.comments || "-"} />
              </div>
              <DetailItem
                label={tr("Sender", "Yuboruvchi", "Отправитель")}
                value={selectedApplication.sendProfileFullName || `${senderDetailQuery.data?.name || ""} ${senderDetailQuery.data?.surname || ""}`.trim() || "-"}
              />
              <DetailItem
                label={tr("Acceptor", "Qabul qiluvchi", "Принимающий")}
                value={selectedApplication.acceptorProfileFullName || `${acceptorDetailQuery.data?.name || ""} ${acceptorDetailQuery.data?.surname || ""}`.trim() || tr("Not assigned yet", "Hali qabul qilinmagan", "Ещё не принято")}
              />
              <DetailItem label={tr("Department", "Bo'lim", "Отдел")} value={selectedApplication.departmentTitle || departmentDetailQuery.data?.title || "-"} />
              <DetailItem label={tr("Building", "Bino", "Здание")} value={selectedApplication.buildingTitle || buildingDetailQuery.data?.title || "-"} />
              <DetailItem label={tr("Category", "Kategoriya", "Категория")} value={selectedApplication.categoryTitle || categoryDetailQuery.data?.title || "-"} />
              <DetailItem label={tr("Service", "Xizmat", "Услуга")} value={selectedApplication.offeringTitle || offeringDetailQuery.data?.title || "-"} />
              <DetailItem label={tr("Created", "Yaratilgan", "Создано")} value={formatDateTime(selectedApplication.createdDate)} />
              <DetailItem label={tr("Updated", "Yangilangan", "Обновлено")} value={formatDateTime(selectedApplication.updatedDate)} />
            </CardContent>
          </Card>
        </div>
      ) : null}
