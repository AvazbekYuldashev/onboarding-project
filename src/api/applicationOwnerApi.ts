...
const APPLICATION_OWNER_BASE_PATH = "/api/v1/application-owner";
...
export async function filterOwnerApplications(
  ...
  try {
    const response = await apiClient.post<unknown>(
      `${APPLICATION_OWNER_BASE_PATH}/filter`,
      payload,
      {
        ...withLanguage(),
        params: { page, size },
...
export async function updateOwnerApplicationStatus(payload: ApplicationStatusDTO): Promise<string> {
  try {
    const response = await apiClient.patch<AppResponse<string>>(
      `${APPLICATION_OWNER_BASE_PATH}/status`,
      payload,
      withLanguage(),
    );
