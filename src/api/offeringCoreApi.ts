...
import type { OfferingPageResponse } from "@/types/offeringManager";

const OFFERING_CORE_BASE_PATH = "/api/v1/offering-core";
...
export async function getOfferingById(id: string): Promise<OfferingResponseDTO> {
  try {
    const response = await apiClient.get<unknown>(
      `${OFFERING_CORE_BASE_PATH}/by-id/${encodeURIComponent(id)}`,
      withLanguage(),
    );
    return normalizeOffering(response.data);
  } catch (error) {
    throw parseAxiosError(error, "Failed to load offering by id.");
  }
}

export async function getOfferingsByDepartment(
  departmentId: string,
  page = 1,
  size = 10,
): Promise<OfferingPageResponse> {
  try {
    const response = await apiClient.get<unknown>(
      `${OFFERING_CORE_BASE_PATH}/by-department/${encodeURIComponent(departmentId)}`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load offerings by department.");
  }
}

export async function getOfferingsByCategory(
  categoryId: string,
  page = 1,
  size = 10,
): Promise<OfferingPageResponse> {
  try {
    const response = await apiClient.get<unknown>(
      `${OFFERING_CORE_BASE_PATH}/by-category/${encodeURIComponent(categoryId)}`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load offerings by category.");
  }
}

export async function getOfferingsByBuilding(
  buildingId: string,
  page = 1,
  size = 10,
): Promise<OfferingPageResponse> {
  try {
    const response = await apiClient.get<unknown>(
      `${OFFERING_CORE_BASE_PATH}/by-building/${encodeURIComponent(buildingId)}`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load offerings by building.");
  }
}
