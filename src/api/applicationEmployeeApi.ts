...
const APPLICATION_EMPLOYEE_BASE_PATH = "/api/v1/application-employee";
...
function normalizeApplication(payload: unknown): ApplicationResponseDTO {
  if (!isRecord(payload)) throw new Error("Invalid application response.");
  return {
...
function normalizeApplicationPage(payload: unknown): ApplicationPageResponse {
  if (!isRecord(payload)) throw new Error("Invalid applications page response.");
...
export async function filterEmployeeApplications(
  ...
  try {
    const response = await apiClient.post<unknown>(
      `${APPLICATION_EMPLOYEE_BASE_PATH}/filter`,
      payload,
      {
        ...withLanguage(),
        params: { page, size },
...
export async function getEmployeeStatusAcceptApplications(
  page = 1,
  size = 10,
): Promise<ApplicationPageResponse> {
  try {
    const response = await apiClient.get<unknown>(
      `${APPLICATION_EMPLOYEE_BASE_PATH}/status-accept`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load incoming applications.");
  }
}

export async function getEmployeeAcceptedApplications(
  page = 1,
  size = 10,
): Promise<ApplicationPageResponse> {
  try {
    const response = await apiClient.get<unknown>(
      `${APPLICATION_EMPLOYEE_BASE_PATH}/by-mee`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load accepted applications.");
  }
}

export async function updateEmployeeApplicationStatus(payload: ApplicationStatusDTO): Promise<string> {
  try {
    const response = await apiClient.patch<AppResponse<string>>(
      `${APPLICATION_EMPLOYEE_BASE_PATH}/status`,
      payload,
      withLanguage(),
    );
