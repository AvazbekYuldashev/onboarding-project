...
const OFFERING_MANAGER_BASE_PATH = "/api/v1/offering-manager";
...
export async function createManagerOffering(payload: OfferingManagerCreateDTO): Promise<OfferingResponseDTO> {
  try {
    const response = await apiClient.post<unknown>(
      `${OFFERING_MANAGER_BASE_PATH}/create`,
      payload,
      withLanguage(),
    );
...
export async function updateManagerOffering(payload: OfferingManagerUpdateDTO): Promise<string> {
  try {
    const response = await apiClient.put<AppResponse<string>>(
      `${OFFERING_MANAGER_BASE_PATH}/update`,
      payload,
      withLanguage(),
    );
