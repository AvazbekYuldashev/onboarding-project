...
const APPLICATION_ADMIN_BASE_PATH = "/api/v1/application-admin";
...
function normalizeApplication(payload: unknown): ApplicationResponseDTO {
  ...
  return {
    ...
    comments: readString(payload.comments) || undefined,
    ...
    sendProfileId: readString(payload.sendProfileId ?? payload.senderProfileId),
    sendProfileFullName: readString(
      payload.sendProfileFullName ??
        payload.senderProfileFullName ??
        payload.sendProfileName ??
        payload.senderName,
    ),
    acceptorProfileId: readString(payload.acceptorProfileId ?? payload.receiverProfileId),
    acceptorProfileFullName: readString(
      payload.acceptorProfileFullName ??
        payload.receiverProfileFullName ??
        payload.acceptorName ??
        payload.receiverName,
    ),
    departmentId: readString(payload.departmentId),
    departmentTitle: readString(payload.departmentTitle ?? payload.departmentName),
    buildingId: readString(payload.buildingId),
    buildingTitle: readString(payload.buildingTitle ?? payload.buildingName),
    categoryId: readString(payload.categoryId),
    categoryTitle: readString(payload.categoryTitle ?? payload.categoryName),
    offeringId: readString(payload.offeringId),
    offeringTitle: readString(payload.offeringTitle ?? payload.offeringName ?? payload.serviceTitle),
...
export async function getAdminApplicationsByBuilding(
  buildingId: string,
  page = 1,
  size = 10,
  ...
  try {
    const response = await apiClient.get<unknown>(
      `${APPLICATION_ADMIN_BASE_PATH}/by-building/${encodeURIComponent(buildingId)}`,
      {
        ...withLanguage(),
        params: { page, size },
      },
    ...
    throw parseAxiosError(error, "Failed to load building applications.");
  }
}

export async function filterAdminApplications(
  payload: ApplicationFilterDTO,
  page = 1,
  size = 10,
): Promise<ApplicationPageResponse> {
  try {
    const response = await apiClient.post<unknown>(
      `${APPLICATION_ADMIN_BASE_PATH}/filter`,
      payload,
      {
        ...withLanguage(),
        params: { page, size },
    ...
    throw parseAxiosError(error, "Failed to load applications.");
  }
}

export async function updateAdminApplicationStatus(payload: ApplicationStatusDTO): Promise<string> {
  try {
    const response = await apiClient.patch<AppResponse<string>>(
      `${APPLICATION_ADMIN_BASE_PATH}/status`,
      payload,
      withLanguage(),
    );
